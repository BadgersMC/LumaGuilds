...
class GuildPartyChatSettingsMenu(private val menuNavigator: MenuNavigator, private val player: Player,
                                 private val guild: Guild, private val party: Party, private val messageService: MessageService) : Menu, KoinComponent {

    override fun open() {
        val gui = ChestGui(6, AdventureMenuHelper.createMenuTitle(player, messageService, "<aqua><aqua>Party Chat Settings"))
        val pane = StaticPane(0, 0, 9, 3)

        val placeholderItem = ItemStack(Material.PAPER)
        val meta = placeholderItem.itemMeta ?: Bukkit.getItemFactory().getItemMeta(Material.PAPER)!!
        meta.setDisplayName("<gray>Chat Settings Feature")
