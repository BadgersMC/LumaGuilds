...
class GuildPartyEventsMenu(private val menuNavigator: MenuNavigator, private val player: Player,
                          private val guild: Guild, private val party: Party, private val messageService: MessageService) : Menu, KoinComponent {

    override fun open() {
        val gui = ChestGui(6, AdventureMenuHelper.createMenuTitle(player, messageService, "<yellow><yellow>Party Events"))
        val pane = StaticPane(0, 0, 9, 3)

        val placeholderItem = ItemStack(Material.BOOK)
        val meta = placeholderItem.itemMeta ?: Bukkit.getItemFactory().getItemMeta(Material.BOOK)!!
        meta.setDisplayName("<gray>Events Feature")
