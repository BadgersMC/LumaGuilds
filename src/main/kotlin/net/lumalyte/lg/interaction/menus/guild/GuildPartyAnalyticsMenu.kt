...
class GuildPartyAnalyticsMenu(private val menuNavigator: MenuNavigator, private val player: Player,
                             private val guild: Guild, private val party: Party, private val messageService: MessageService) : Menu, KoinComponent {

    override fun open() {
        val gui = ChestGui(6, AdventureMenuHelper.createMenuTitle(player, messageService, "<light_purple><light_purple>Party Analytics"))
        val pane = StaticPane(0, 0, 9, 3)

        val placeholderItem = ItemStack(Material.KNOWLEDGE_BOOK)
        val meta = placeholderItem.itemMeta ?: Bukkit.getItemFactory().getItemMeta(Material.KNOWLEDGE_BOOK)!!
        meta.setDisplayName("<gray>Analytics Feature")
