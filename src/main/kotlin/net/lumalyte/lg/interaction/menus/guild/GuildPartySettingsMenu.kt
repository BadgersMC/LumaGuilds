...
class GuildPartySettingsMenu(private val menuNavigator: MenuNavigator, private val player: Player,
                            private val guild: Guild, private val party: Party, private val messageService: MessageService) : Menu, KoinComponent {

    override fun open() {
        val gui = ChestGui(6, AdventureMenuHelper.createMenuTitle(player, messageService, "<gold><gold>Party Settings"))
        val pane = StaticPane(0, 0, 9, 3)

        val placeholderItem = ItemStack(Material.CRAFTING_TABLE)
        val meta = placeholderItem.itemMeta ?: Bukkit.getItemFactory().getItemMeta(Material.CRAFTING_TABLE)!!
        meta.setDisplayName("<gray>Settings Feature")
