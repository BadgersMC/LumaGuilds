package net.lumalyte.lg.infrastructure.persistence.partitions
...
class PartitionRepositorySQLite(private val storage: Storage<Database>): PartitionRepository {
    ...
    override fun getAll(): Set<Partition> {
        val result = partitions.values.toSet()
        // Return all partitions from memory cache
        return result
    }
    ...
    override fun getByChunk(position: Position): Set<Partition> {
        ...
        val localChunkPartitions = chunkPartitions[position]

        if (localChunkPartitions == null) {
            // No partitions found for this chunk
            return setOf()
        }
    ...
    private fun preload() {
        try {
            ...
            println("[ERROR] PartitionRepositorySQLite.preload: Failed to preload partitions: ${error.message}")
            error.printStackTrace()
        }
    }
}
