    ...
    private fun loadQuests(): QuestSystemConfig {
        ...
        return QuestSystemConfig(
            ...
            generation = QuestGenerationConfig(
                ...
                secondConditionChancePercent = yamlConfig.getInt("$base.generation.second_condition_chance_percent", 15).coerceIn(0, 100),
                maxConditions = yamlConfig.getInt("$base.generation.max_conditions", 2).coerceIn(0, 4),
                exactRepeatCooldownWeeks = yamlConfig.getInt("$base.generation.exact_repeat_cooldown_weeks", 8).coerceAtLeast(0),
                actionTargetCooldownWeeks = yamlConfig.getInt("$base.generation.action_target_cooldown_weeks", 3).coerceAtLeast(0),
                axisCenters = yamlConfig.getIntegerList("$base.generation.axis_centers").ifEmpty {
                    listOf(0, 0, 0, 0, 500, -500, 1000, -1000, 2500, -2500)
                },
                axisWidths = yamlConfig.getIntegerList("$base.generation.axis_widths").filter { it > 0 }.ifEmpty {
                    listOf(50, 100, 250, 500)
                },
                yLevels = yamlConfig.getIntegerList("$base.generation.y_levels").ifEmpty {
                    listOf(-32, 0, 32, 64, 96, 128)
                }
            )
        )
    }
